...
def pack_sample(time_delta: int, lat_delta: int, lon_delta: int, flags: int, extra_absolute: int) -> bytes:
    return struct.pack(
        '>BHhhh',
        flags,           # 1 byte:  Flags
...
def pack_header(ref_timestamp: int, ref_lat_fixed: int, ref_lon_fixed: int, sample_count: int) -> bytes:
    return struct.pack(
        '>HBiiiB',
...
def unpack_header(payload: bytes) -> Tuple[int, int, int, int, bytes]:
    ...
    protocol, protocol_version, ref_timestamp, ref_lat_fixed, ref_lon_fixed, sample_count = header_data
    ...
    return (ref_timestamp, ref_lat_fixed, ref_lon_fixed, sample_count, payload[OUTPOST_HEADER_WIDTH:])

def unpack_sample(samples: bytes, reference: FixedPositionSample) -> Tuple[FixedPositionSample, bytes]:
    flags, time_delta, lat_delta, lon_delta, extra = struct.unpack('>BHhhh', samples[:OUTPOST_SAMPLE_WIDTH])
