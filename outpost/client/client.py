...
class OutpostClient:
    def __init__(self, device: str, baud: int, outpost_host: str, psk: bytes, similarity_threshold: float):
        self.device = device
        self.baud = baud
    ...
    def start(self) -> None:
        ...
        self.collection_thread = threading.Thread(target=self.collection_task.run, daemon=True)
        self.collection_thread.start()
        
        # Start processing thread
        self.processing_thread = threading.Thread(target=self.processing_task.run, daemon=True)
...
def main() -> None:
    ...
    parser.add_argument('--psk', '-p', required=True, help='Path to pre-shared key file for encryption')
