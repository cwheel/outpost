    ...
    async def secure_render_post(self, request: Message) -> Message:
        try:
            ...
            return aiocoap.Message(
                mtype=aiocoap.NON, code=aiocoap.INTERNAL_SERVER_ERROR
            )


class OutpostServer:
    def __init__(
        self: Self, host: str, port: int, database_url: str, psk: bytes
    ) -> None:
        self.host = host
        self.port = port
...
async def main() -> None:
    ...
    database_url = os.getenv(
        "DATABASE_URL", "postgresql://outpost:outpost@postgres/outpost"
    )
