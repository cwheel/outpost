    ...
    for i, sample in enumerate(fixed_samples):
        ...
        if i == 0:
            ...
            if not (-32768 <= lat_delta <= 32767):
                raise OverflowError(f"Latitude delta {lat_delta} exceeds 16-bit signed range")
    
            if not (-32768 <= lon_delta <= 32767):
                raise OverflowError(f"Longitude delta {lon_delta} exceeds 16-bit signed range")
            
            previous_sample = sample

        time_delta = sample['time'] - ref_timestamp
        
        sample_data = pack_sample(time_delta, lat_delta, lon_delta, sample['flags'], sample['extra'])
...
def unpack_batch(packed_data: bytes) -> List[PositionSample]:
    ref_timestamp, ref_lat_fixed, ref_lon_fixed, sample_count, packed_samples = unpack_header(packed_data)
